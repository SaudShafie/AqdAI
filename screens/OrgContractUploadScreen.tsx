...
type Props = NativeStackScreenProps<RootStackParamList, 'OrgContractUpload'>;
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      Alert.alert('Success', 'Contract uploaded successfully!', [
        { text: 'OK', onPress: () => navigation.navigate('OrgContractList') }
      ]);
  ...
  const loadCategories = async () => {
    try {
      ...
      if (!userWithData?.userData?.organizationId) {
        setCategories(['Employment', 'Rental', 'Service Agreement', 'Purchase', 'Partnership', 'NDA', 'Other']);
        return;
      }
      ...
      const categoriesQuery = query(
        categoriesRef,
        where('organizationId', '==', userWithData.userData.organizationId)
      );
      const categoriesSnapshot = await getDocs(categoriesQuery);
      
      const orgCategories = categoriesSnapshot.docs.map(doc => doc.data().displayName || doc.data().name);
      
      // Load categories from existing contracts
      ...
      contractsSnapshot.docs.forEach(doc => {
        ...
        if (contractData.category && contractData.category.trim()) {
          contractCategories.add(contractData.category.trim());
        }
      });

      // Combine and deduplicate categories
      const allCategories = [...new Set([...orgCategories, ...contractCategories, 'Employment', 'Rental', 'Service Agreement', 'Purchase', 'Partnership', 'NDA', 'Other'])];
            ...
            <View style={styles.structuredFieldsContainer}>
              <View style={styles.fieldContainer}>
                ...
                <TextInput
                  ...
                  placeholder="e.g., Brief overview of the contract"
                  placeholderTextColor="#999"
                  value={summary}
...
const styles = StyleSheet.create({
  ...
  // New styles for structured fields
  ...
  structuredInput: {
    ...
    backgroundColor: 'white',
    color: '#333',
    minHeight: 80, // Adjust as needed for multiline
    textAlignVertical: 'top',
  },
  optionalText: {
    fontSize: 10,
